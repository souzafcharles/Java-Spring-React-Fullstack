...
- Implement methods to retrieve all `Food` entities:
    ...
    - Use `@Transactional(readOnly = true)` to ensure the method runs within a read-only transaction for optimized
      database performance.
- Implement a method to insert a new `Food` entity:
    - `insert`: Saves a new `Food` entity in the database;
    - Use `@Transactional` to ensure that this method runs within a transactional context, enabling database operations
      rollback if any exceptions occur.
...
- This section covers the initialization of the `tb_foods` table with sample food data in both `H2 Database` and
  `PostgreSQL`;
- It includes the necessary `SQL` commands to create the table and insert predefined food records, ensuring a consistent
  dataset
...
- This section covers the implementation of success cases when requesting and responding to food data through a Spring
  Boot RESTful API. It details how different HTTP methods interact with the Food entity, ensuring proper retrieval,
...
- This section covers the implementation of exception handling for the `findById` operation in the `FoodService` class,
  introducing custom exceptions and centralized error handling mechanisms.
...
- **Scenario**: The requested ID `17` does not exist, triggering the custom error response with a `404 Not Found` status
  code:
...
This section covers the implementation of exception handling for the `delete` operation in the `FoodService` class,
introducing custom exceptions and centralized error handling mechanisms.
...
- New Method: `handleDatabaseException`(DatabaseException e, HttpServletRequest request) handles database-related
  exceptions.
...
- Modify the `delete` method to throw the custom `ResourceNotFoundException`, `DatabaseException`, and
  `EmptyTableException`:
...
- **Key Attributes Explained:**
    - Ensure that `ResourceNotFoundException` is thrown when the provided `id` does not exist;
    - Maintain robust exception handling by catching `DataIntegrityViolationException`,
      `InvalidDataAccessResourceUsageException`, and throwing `EmptyTableException` for invalid table access cases.
...
- **Scenario**: The requested ID `1` exists, but due to relationships with another entity, a
...
This section covers the implementation of exception handling for the `update` operation in the `FoodService` class,
introducing custom exceptions and centralized error handling mechanisms.
...
- This section covers the implementation of exception handling for the `insert` and `findAll` operations in the
  `FoodService` class, introducing custom exceptions and centralized error handling mechanisms.
...
- Custom exception created to handle cases where the database table is empty or does not exist during the `findAll`
  operation;
- Constructor:
...
#### 9.3. **UPDATE CLASS**: `controller.exceptions.ResourceExceptionHandler`:

- **Updated Method:** handleResourceNotFound(`ResourceNotFoundException` e, HttpServletRequest request) handles
  resource-not-found exceptions.
- **Updated Method:** handleDatabaseException(`DatabaseException` e, HttpServletRequest request) handles
  database-related exceptions.
- **New Method:** handleEmptyTableException(`EmptyTableException` e, HttpServletRequest request) handles empty table
  exceptions.
- **New Method:** handleBadRequestException(`InvalidDataException` e, HttpServletRequest request) handles invalid data
  input exceptions.

**Removed Methods:**

#### 9.4. **UPDATE METHOD**: `insert` in `FoodService`:

- Modify the `insert` method to throw the custom `InvalidDataException`:
...
#### 9.5. **UPDATE METHOD**: `findAll` in `FoodService`:
...
#### 9.6. Error Case: Handling Resource Not Found via Spring Boot RESTful API (`insert`):

#### 9.6.1. Setting Up the RESTful API for HTTP Methods (Idempotent):

- **Endpoint**: POST `/foods`;
- **Purpose**: Inserts a new Food item into the database.

#### 9.6.2. Example POST Request (Invalid Data):
...
#### 9.6.3. Example Error Response:

````json
{
  "timestamp": "2025-02-05T20:36:09Z",
  "status": 400,
  "error": "Invalid data provided. Please check the input and try again.",
  "message": "Data integrity violation. Check the provided data.",
  "path": "/foods"
}
````

#### 9.7. Error Case: Handling Resource Not Found via Spring Boot RESTful API (`findAll`):

#### 9.7.1. Setting Up the RESTful API for HTTP Methods (Idempotent):

- **Endpoint** GET `/foods`;
- **Purpose**: Retrieves all Food items from the database.

#### 9.7.2. Example GET Request (Empty Table):
...
#### 9.7.3. Example Error Response:

````json
{
  "timestamp": "2025-02-05T21:09:02Z",
  "status": 404,
...
- `timestamp`: Indicates when the error occurred;
- `status`: The HTTP status code (either `404` for `Not Found`);
- `error`: Short description of the issue;
- `message`: Detailed information, including the resource identifier;
- `path`: The URI path of the failed request.
...
- This section covers the process of populating the database with additional food items using SQL scripts for both H2
...
- [X] Create custom exceptions: `ResourceNotFoundException`, `DatabaseException`, `EmptyTableException` and
...
- [X] Implement methods in `ResourceExceptionHandler` to handle specific exceptions (`ResourceNotFoundException`,
  `DatabaseException`, `EmptyTableException` and `InvalidDataException);
- [X] Update service methods (`findById`, `delete`, `update`, `insert`, `findAll`) to throw custom exceptions.

**Success and Error Scenarios:**

- [X] Test `GET /foods` for retrieving all foods (success and empty table cases);
- [X] Test `POST /foods` for inserting a new food (success and invalid data cases);
- [X] Test `GET /foods/{id}` for retrieving a specific food (success and resource not found cases);
- [X] Test `PUT /foods/{id}` for updating a specific food (success and resource not found cases);
- [X] Test `DELETE /foods/{id}` for deleting a specific food (success, resource not found, and data integrity violation
  cases).